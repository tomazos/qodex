...
void CliDispatcherTest::recognizesCliInvocationsFromRawArguments() {
    ...
    QVERIFY(dispatcher.shouldRunCli({QStringLiteral("--debug"), QStringLiteral("db"), QStringLiteral("query")}));
...
void CliDispatcherTest::rendersHelpCommandOutput() {
    ...
    const int exitCode = helpCommand->run(
        ...
        qodex::cli::CliContext{
            ...
            .databasePath = QStringLiteral("/tmp/qodex.sqlite3"),
    ...
    QVERIFY(stdoutText.contains(QStringLiteral("--debug")));
}
